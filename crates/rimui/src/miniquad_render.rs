...
pub struct MiniquadRender<'b, 'f, S> 
    where S: Fn(SpriteKey)->miniquad::Texture {
...
impl<'b, 'f, S> Render for MiniquadRender<'b, 'f, S> 
    ...
    fn draw_rounded_rect(&mut self, rect: [f32; 4], radius: f32, thickness: f32, outline_color: [u8; 4], fill_color: [u8; 4]) {
        self.batch.geometry.fill_round_rect_aa(vec2(rect[0], rect[1]), vec2(rect[2], rect[3]), radius, 8, fill_color);
        self.batch.geometry.stroke_round_rect_aa(vec2(rect[0], rect[1]), vec2(rect[2], rect[3]), radius, 8, thickness, outline_color);
    }
}
